...
export function SystemHealthHeader({ title = "SYMBIOSIS Mission Control" }: { title?: string }) {
  ...
  return (
    <header className="flex items-center justify-between px-gutter py-3 h-16 z-50 bg-paper-surface/95 backdrop-blur-md border-b border-on-surface-variant/40 shrink-0 sticky top-0">
      ...
      <div className="flex items-center gap-3 md:gap-4">
        <div className="flex items-center gap-2 bg-surface-container-low/90 border border-outline-variant/60 rounded-lg px-3 py-1.5 shadow-sm">
          ...
          <Link 
            to="/perception"
            className="flex items-center gap-1.5 text-xs font-label-caps text-on-surface hover:opacity-80 transition-opacity select-none"
            title="ResNet-50 Neural Inference Engine (Click to inspect Perception)"
          >
            <span
              className={`w-2.5 h-2.5 rounded-full ${
        ...
        <button
          onClick={handleManualRefresh}
          className="text-on-surface-variant hover:text-lacquer-red transition-all p-2 rounded-lg bg-surface-container-low/90 border border-outline-variant/60 hover:border-lacquer-red/50 shadow-sm active:scale-95"
